...
build_kernel() {
	...
	tg_post_msg "<code>|| Started Compile!! ||</code>"
	make -j"$PROCS" O=out \
	...
	then
		...
		then
			...
			tg_post_msg "<code>|| Kernel Successfully Compiled!! ||</code>"
			if [ $BUILD_DTBO = 1 ]
			then
				msg "|| Building DTBO ||"
...
gen_zip() {
	...
	then
		...
		if [ "$PTTG" = 1 ]
 		then
 			msg "|| Signing Zip ||"
			tg_post_msg "<code>|| Signing Zip file to Telegram!! ||</code>"
 		fi
